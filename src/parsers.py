...
def get_or_create_workout(workouts: Dict[Tuple[str, str], Workout], key: Tuple[str, str], name: str, date: datetime, duration: int, notes: str) -> Workout:
...
def get_or_create_exercise(workout: Workout, exercise_name: str, body_part: Optional[BodyPart]) -> Exercise:
...
def parse_csv(file_path: str, mappings_path: Optional[str] = None) -> List[Workout]:
    """
    Parses CSV from Strong app. Handles inserting data into a dictionary of Workouts, with key (date, workout name).
    """
    mappings = load_mappings(mappings_path) if mappings_path else load_mappings("../data/exercise_body_part_mapping.json")
    ...
    with open(file_path, mode="r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            workout = get_or_create_workout(workouts, workout_key, workout_name, parsed_date, duration, workout_notes)
